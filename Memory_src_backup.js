...
function vulSpeelveld(size) {
    var tabel = document.getElementById("speelveld");
    karakter = document.getElementById("character");
    karakter = karakter.options[karakter.selectedIndex].value;
    tabel.innerHTML = "";

    nLetter = nextLetter(size)

    for (var i = 0; i < size; i++) {
        var tr = document.createElement("tr");
        for (var j = 0; j < size; j++) {
            var td = document.createElement("td");
            td.appendChild(document.createTextNode(karakter));
            td.hiddenValue = nLetter();
...
function cardClicked(card) {
    checkStarttijd();
    checkDerdeKaart();
    var draaiKaartOm = turnCard(this);
    if (draaiKaartOm == 2) {
        checkKaarten();
    }
}
...
function toggleCard(element) {
    ...
    if (element.className == "active") {
        ...
        element.replaceChild(document.createTextNode(karakter), element.childNodes[0])
    } else {
        element.className = "active"
        element.replaceChild(document.createTextNode(element.hiddenValue), element.childNodes[0])
    }
}
